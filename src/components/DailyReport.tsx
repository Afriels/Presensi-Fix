    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            {user?.role === 'admin' && (
              <button onClick={() => handleOpenModal(null)} className="flex-1 sm:flex-none flex items-center justify-center px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition">
                <PlusIcon className="h-4 w-4 mr-2" />
                Tambah
              </button>
            )}
            <button onClick={exportToCSV} className="flex-1 sm:flex-none px-4 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 transition">
              Export
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-4">
        <div className="flex-1">
          ...
          <input type="date" id="date-filter" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm rounded-md" />
        </div>
        <div className="flex-1">
                  ...
                  <tr key={student.id}>
                    ...
                    {user?.role === 'admin' && (
                        ...
                        <div className="flex justify-end items-center gap-2">
                            <button onClick={() => handleOpenModal(record)} className="p-2 text-sky-600 hover:text-sky-900 hover:bg-gray-100 rounded-full"><PencilIcon className="h-4 w-4"/></button>
              ...
              <div key={student.id} className="bg-white p-4 rounded-lg shadow">
                ...
                {user?.role === 'admin' && (
                    <div className="mt-4 flex justify-end items-center gap-2 border-t pt-2">
                        <button onClick={() => handleOpenModal(record)} className="p-2 text-sky-600 hover:bg-gray-100 rounded-full"><PencilIcon className="h-5 w-5"/></button>
                        { 'id' in record && record.id &&
